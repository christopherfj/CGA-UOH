...
class Viewer3DWidget(QtOpenGL.QGLWidget):
    ...
    def keyPressEvent(self, evento):
        if evento.key() == Qt.Key_Right:
            self.escalamiento = (0.5, 0.5)
        elif evento.key() == Qt.Key_Left:
            self.escalamiento = (2.0,2.0)
        self.dibujar()
        self.updateGL() 
...
class Ventana(QtWidgets.QMainWindow):    
    ...
    #slots
    @pyqtSlot()
    def agrandar(self, sx=2.0, sy=2.0):
        self.viewer3D.escalamiento = (sx, sx)
        self.viewer3D.updateGL() 
    @pyqtSlot()
    def achicar(self, sx=0.5, sy=0.5):
        self.viewer3D.escalamiento = (0.5, 0.5)
        self.viewer3D.updateGL() 
    @pyqtSlot()
    def original(self,sx=1.0, sy=1.0):
        self.viewer3D.escalamiento = (sx,sy)
        self.viewer3D.updateGL() 
